...
I need to reformat my data again- changing the names so that I can plot all of them simultaneously much more easily
```{r}
...
ULB.day <- filter(day, shed == "ULB")
GG.day <- filter(day, shed == "GG")
RMO.day <- filter(day, shed == "RMO")
MUL.day <- filter(day, shed == "MUL")

par(mfrow = c(2,2))
for (i in 1:length(sheds)) {
  f <- filter(day, shed == sheds[i])  
  plot <- ggplot(data = f)+
    geom_line(aes(x = rnd.dates,
                  y = d.mean,
                  color = site))
