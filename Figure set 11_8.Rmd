...
plotJPwants <- ggplot(modeltester, aes(x = avg.head,
                                       y = h.avg.model))+
  theme_classic()+
  geom_point(aes(color = shed), size = 3)+
   #geom_smooth(method = 'lm', se = FALSE)+
...
slope <- read.csv("maybemosslope.csv")
elev <- read.csv("maybemoselev.csv")
maye <- read.csv("maybe_moses.csv")
...
maybe <- select(maye, Site, type, uaa_ft2)

mosay <- left_join(maybe, slope, by = "Site")
mosay <- left_join(mosay, elev, by = "Site")
```
...
modelmaybemose <- left_join(maye, mostest, by = "Site")%>%
  select(Site, type.x, NEAR_X, NEAR_Y, h.avg.model)

mop <- modelmaybemose[complete.cases(modelmaybemose),]
#write.csv(mop, file = "modelmaybemoses_11_14.csv")
```
