...
unique((agg.tall.1$site[is.na(agg.tall.1$d.avg.airtemp)]))
...
ggplot(agg.tall.1, aes(x = d.avg.airtemp))+
  geom_line(aes(y = value))+
  facet_wrap(metric ~., free)+
  theme_classic()

ggplot(atak.day, aes(x = day, y = d.avg.headtemp))+
  geom_line()
  

test <- read.csv("pred.coeffs.3_11.csv")
```
...
FIGURE 6: DAILY MEAN PREDICTION R2
```{r}
#use pred.coeffs.3_11.csv, it is the output of the daily mean prediction stuff
preds <- read.csv("pred.coeffs.3_11.csv")

r2.data <- filter(preds, metric %in% c("r2.elev", "r2.slope", "r2.uaa"))

r2.plot <- ggplot(r2.data, aes(x = d.avg.airtemp,
                               y = value))+
  geom_point()
#old code below- feel free to delete later
agg.tall.1 <- filter(agg.tall, site != "RMO2")%>%
  filter(site != "RMO3")%>%
  filter(site != "RMO1")%>%
  filter(site != "MUL2")
